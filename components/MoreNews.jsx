...
export default function MoreNews({ moreNews }) {
    ...
    const moreNewsTitle = "mx-auto w-fit m-2 text-xl font-semibold uppercase text-neutral-800 lg:w-[calc(15rem*4+3rem)]"
        ...
        {moreNews.map(item => (
            <article key={item.id} className="mb-10 relative">
                <Link href={`/news-details/${item.slug}`}>
                    ...
                    <h2 className="w-60 text-sm mt-1 neutral-800 hover:underline line-clamp-2">
                        {item.title}
                    </h2>
