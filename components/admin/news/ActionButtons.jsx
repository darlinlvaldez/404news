...
export const ActionButtons = ({ 
  ...
  if (!showDeleteConfirm) {
    return (
      <div className="flex flex-col sm:flex-row items-center gap-4">
        ...
        {canDelete && (
          <button onClick={() => onSetShowDeleteConfirm(true)} className="w-full cursor-pointer sm:w-auto text-red-500 hover:bg-red-500/10 font-bold px-8 py-5 rounded-3xl">
            Eliminar Entrada
          </button>
        )}
      </div>
  ...
  return (
      ...
      <h4 className="text-2xl font-black mb-2 text-white">¿Eliminar noticia?</h4>
      ...
      <div className="flex space-x-4 w-full max-w-xs">
        <button 
          ...
          className="flex-1 cursor-pointer bg-gray-700 hover:bg-gray-600 px-6 py-3 rounded-2xl font-bold transition active:scale-95"
        >
          Cancelar
        </button>
        <button 
          className="flex-1 cursor-pointer bg-red-600 hover:bg-red-500 px-6 py-3 rounded-2xl font-bold transition shadow-lg shadow-red-900/40 active:scale-95"
          onClick={onDelete}
        >
          Sí, Eliminar
        </button>
