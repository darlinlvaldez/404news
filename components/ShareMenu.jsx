...
function getShareStyles(open) {
  ...
  const shareItemBase = `w-12 h-12 bg-green-700 rounded-full flex items-center justify-center 
  transition-all duration-300 ease-out hover:bg-green-600`;

  const shareItemAnimation = open ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2";
...
export default function ShareMenu({ slug }) {
  ...
  return (
    <div className="absolute right-0 top-1/2 -translate-y-1/2 text-white">
      ...
      <div className={shareMenuStyles}>
        <a href={`https://twitter.com/intent/tweet?url=${shareUrl}`} target="_blank" rel="noopener noreferrer"
          className={`${shareItemBase} delay-75 ${shareItemAnimation}`} aria-label="Compartir en X">
        ...
        <a href={`https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`} target="_blank" rel="noopener noreferrer"
          className={`${shareItemBase} delay-150 ${shareItemAnimation}`} aria-label="Compartir en Facebook">
        ...
        <a href="https://www.instagram.com/" target="_blank" rel="noopener noreferrer"
          className={`${shareItemBase} delay-200 ${shareItemAnimation}`} aria-label="Compartir en Instagram">
        ...
        <a href={`mailto:?subject=Noticia Interesante&body=${shareUrl}`}
          className={`${shareItemBase} delay-300 ${shareItemAnimation}`} aria-label="Compartir por email">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
        ...
        <button onClick={() => { navigator.clipboard.writeText(shareUrl);
          ...
          className={`${shareItemBase} delay-[400ms] ${shareItemAnimation}`} aria-label="Copiar enlace">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5">
